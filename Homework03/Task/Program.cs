...
using System.Linq;
using Task.Enums;
using static Task.Dog;
using static Task.Person;
...
namespace Task
{
    class Program
    {
        private static void Main(string[] args)
		{
			...
			IEnumerable<Person> allPersonFirstNameWithR = from person in people
														  where person.FirstName.StartsWith("R")
														  orderby person.Age descending
														  select person;
			Console.WriteLine("Task 1.");

            foreach (var person in allPersonFirstNameWithR)
            {
				Console.WriteLine($"Person named {person.FirstName} is {person.Age} years old.");
            }

			//Find and print all brown dogs names and ages older than 3 years, ordered by Age - ASCENDING ORDER

			IEnumerable<Dog> brownDogsNamesAndAges = from dog in dogs
													 where dog.Age > 3
			...
			foreach (var dog in brownDogsNamesAndAges)
            {
				Console.WriteLine($"Dog named {dog.Name} is {dog.Age} years old.");
            }

			// Find and print all persons with more than 2 dogs, ordered by Name - DESCENDING ORDER

			IEnumerable<Person> personWithManyDogs = from person in people
			...
			foreach (var person in personWithManyDogs)
            {
				Console.WriteLine($"{person.FirstName} has more than 2 dogs.");
            }

			// Find and print all Freddy`s dogs names older than 1 year

			var allFreddysDogs = people.Where(person => person.FirstName == "Freddy")
				.SelectMany(dog => dog.Dogs)
				.Where(dogs => dogs.Age > 1);

			Console.WriteLine("Task 4.");

			foreach (var dog in allFreddysDogs)
            {
				Console.WriteLine($"{dog.Name} has {dog.Age} years.");
            }

			// Find and print Nathen`s first dog

			var nathenFirstDog = people.Where(person => person.FirstName == "Nathen")
				.Select(dog => dog.Dogs[0]);

			Console.WriteLine("Task 5.");

			foreach (var dog in nathenFirstDog)
            {
				Console.WriteLine($"Nathen's first dog is {dog.Name}.");
            }

			// Find and print all white dogs names from Cristofer, Freddy, Erin and Amelia, ordered by Name -ASCENDING ORDER

			var allWhiteDogsOrderedByName = people
				.Where(person => person.FirstName == "Cristofer" || person.FirstName == "Freddy" || person.FirstName == "Erin" || person.FirstName == "Amelia")
				.SelectMany(dog => dog.Dogs)
			...
			foreach (var dog in allWhiteDogsOrderedByName) 
            {
				Console.WriteLine($"{dog.Name} is a {dog.Color} color dog.");
            }
		} 
